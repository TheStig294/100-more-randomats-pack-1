...
hook.Add("TTTHUDInfoPaint", "RandomatEventTrackingHUDInfoPaint", function(client, label_left, label_top, active_labels)
    ...
    local text = LANG.GetParamTranslation("rdmt_count_hud", {
        count = #Randomat.ActiveEvents
    })
    ...
    for _, v in ipairs(tbl) do
        if pred(v) then
            if not v.RdmtSmokeEmitter then
                v.RdmtSmokeEmitter = ParticleEmitter(v:GetPos())
            end
...
function Randomat:GetWeaponName(item)
    ...
    if wep and (wep.PrintName or wep.GetPrintName) then return LANG.TryTranslation(wep.PrintName or wep:GetPrintName()) end

    -- Sometimes the direct methods don't seem to work, so we'll do a loop if they fail
    for _, v in ipairs(weapons.GetList()) do
        if item == WEPS.GetClass(v) then return LANG.TryTranslation(v.GetPrintName and v:GetPrintName() or v.PrintName or item) end
    end
