local EVENT = {}
local zonesCvar = CreateConVar("randomat_battleroyale2_storm_zones", 4, FCVAR_ARCHIVE, "The number of zones until the storm covers the map (Set to 0 to disable)", 0, 10)
local waitTimeCvar = CreateConVar("randomat_battleroyale2_storm_wait_time", 30, FCVAR_ARCHIVE, "Seconds after the next zone is announced before the storm moves", 0, 120)
local moveTimeCvar = CreateConVar("randomat_battleroyale2_storm_move_time", 30, FCVAR_ARCHIVE, "Seconds it takes for the storm to move", 0, 120)
local musicCvar = CreateConVar("randomat_battleroyale2_music", 1, FCVAR_ARCHIVE, "Play victory royale music when someone wins", 0, 1)
